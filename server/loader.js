...
import { setCSRF} from '../src/app/actions/csrfActions';
import {
  setCurrentUser,
  clearCurrentUser
...
export default (req, res,next) => {
  ...
  */
  ...
  if (!keys.mode.active) {
    const PathMaintance = path.resolve(__dirname, 'maintanice.html');
    const html = fs.readFileSync(PathMaintance, 'utf8');
  ...
  fs.readFile(
    ...
    (err, htmlData) => {
      ...
      if(!req.user){
        store.dispatch(clearCurrentUser(req.user));
      }
      ...
      // if ('hammerstoutdenim' in req.cookies) {
